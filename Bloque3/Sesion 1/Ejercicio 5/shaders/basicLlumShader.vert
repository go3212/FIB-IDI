...
{	
    ...
    vec3 light = normalize((view*vec4(posFocus, 1.0)).xyz - vertSCO.xyz);
    vec3 normSCO = normalize(inverse(transpose(mat3(view*TG)))*normal);

    fcolor =  Ambient() + Difus(normSCO, light, colorFocus) + Especular(normSCO, light, vertSCO, colorFocus);
