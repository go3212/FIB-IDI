...
BL3GLWidget::BL3GLWidget (QWidget* parent) : QOpenGLWidget(parent), program(NULL)
{
  ...
  posLight = glm::vec3(0.0f, 1.0f, 1.0f);
  colorLight = glm::vec3(0.8f, 0.8f, 0.8f);
  focusMode = 0;
}
...
void BL3GLWidget::iniMaterialTerra ()
{
  ...
  amb = glm::vec3(0,0,0.3);
  diff = glm::vec3(0,0,0.6);
  spec = glm::vec3(1,1,1.0);
  shin = 10;
}
...
void BL3GLWidget::paintGL () 
{
  ...
  glUniform3fv(posLightPos, 1, &posLight[0]);
  glUniform3fv(colorLightPos, 1, &colorLight[0]);
...
void BL3GLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F:
      focusMode = !focusMode;
      glUniform1i(focusModeLoc, focusMode);
      break;
...
void BL3GLWidget::carregaShaders()
{
  ...
  posLightPos = glGetUniformLocation(program->programId(), "posFocus");
  colorLightPos = glGetUniformLocation(program->programId(), "colorFocus");
  focusModeLoc = glGetUniformLocation(program->programId(), "focusMode");
}
